...
class StepnRequest(object):
    ...
    def ensure_connection(self):
        ...
        try:
            ...
            self.get_login(self.__email, password=self.__password)
    ...
    @http_stepn_watcher
    def get_login(self, email: str, password):
        encoded_password = stepn_password.hash_password(email, password)

        url = self.creates_url_params(endpoint='login', account=email, password=encoded_password, type=3)
    ...
    def get_userbasic(self):
        url = self.creates_url_params(endpoint='userbasic')
    ...
    def get_orderlist(self, **kwargs):
        ...
        """
        url = self.creates_url_params(endpoint='orderlist', **kwargs)
    ...
    def get_orderdata(self, order_id):
        ...
        """
        url = self.creates_url_params(endpoint='orderdata', orderId=order_id)
        response = self.session.get(url)
        return response
    ...
    @staticmethod
    def get_orderdata_attrs(row, attr):
        index = mapping_response_attrs_reversed[attr]
        attrs = row.get('data').get('attrs')
        return f"{attrs[index] if len(attrs) > 0 else 'NA'}"
    ...
    def replace_detail_binded_vars(cls, conditions, row):
        ...
        while "%" in conditions:
            ...
            conditions = conditions.replace(f"%{var_to_replace}", real_value)
