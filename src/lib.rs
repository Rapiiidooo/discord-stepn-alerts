...
#[pyfunction]
pub fn hash_password(email: String, password: String) -> PyResult<String> {
    let email_ref = email.clone();
    let password_ref = password.clone();
    const SALT: &[u8] = b"helloSTEPN";
    let email_hash = hash_code(&email_ref);
    ...
    Ok(encode_with_seed(data, email_hash))
}

#[pymodule]
fn stepn_password(_py: Python<'_>, m: &PyModule) -> PyResult<()> {
